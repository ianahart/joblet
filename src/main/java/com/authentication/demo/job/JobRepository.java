...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    @Query(value = """
            SELECT new com.authentication.demo.job.dto.JobDto(
            j.id, j.position, j.perHour, j.body, j.createdAt, j.multipleCandidates,
            j.urgentlyHiring, j.availability, e.id, e.companyName, e.location)
            from Job j INNER JOIN j.employer e WHERE e.id = :employerId ORDER BY e.createdAt""")
    ...
    @Query(value = """
            ...
            e.companyName, e.location) from Job j INNER JOIN j.employer e ORDER BY e.createdAt""")
