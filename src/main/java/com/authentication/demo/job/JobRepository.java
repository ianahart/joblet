...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    @Query(value = "SELECT new com.authentication.demo.job.dto.JobDto(j.id, j.position, j.perHour, j.body, j.createdAt, j.multipleCandidates, j.urgentlyHiring, j.availability, e.id, e.companyName, e.location) from Job j INNER JOIN j.employer e ORDER BY e.createdAt")
    ...
    @Query(value = """
            ...
            e.email, e.firstName, e.lastName, e.numOfEmployees) from Job j INNER JOIN j.employer e WHERE j.id = :employerJobId ORDER BY j.createdAt""")
    ...
    ViewJobDto findJobById(@Param("jobId") Long jobId);

    @Query(value = """
            SELECT
            new com.authentication.demo.job.dto.SyncJobDto(j.position, j.perHour,
             j.availability, j.urgentlyHiring, j.multipleCandidates, j.body, e.id)
            FROM Job j INNER JOIN j.employer e WHERE j.id = :jobId""")
